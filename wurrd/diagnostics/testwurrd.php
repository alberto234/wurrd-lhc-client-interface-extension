...
?>
	...
	<form action="" method="post">
	  Server URL: <input type="text" name="chaturl" size="100" placeholder="http://wurrdapp.com/chat"><br /><br />
...
function getServerInfo($chatURL) {
	$curl = curl_init();
	
	curl_setopt_array($curl, array(
	  CURLOPT_URL => $chatURL . "/index.php/wurrd/clientinterface/serverinfo",
...
function testLogin($chatURL, $username, $password, $deviceUUID) {
	$usePost = isset($_POST['usepost']) && $_POST['usepost'] == "true" ? true : false;
	
	$curl = curl_init();
	
	curl_setopt_array($curl, array(
	  CURLOPT_URL => $chatURL . "/index.php/wurrd/clientinterface/operator/login",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
...
function testCheckForUpdates($chatURL, $accessToken) {
	$curl = curl_init();
	
	curl_setopt_array($curl, array(
	  CURLOPT_URL => $chatURL . "/index.php/wurrd/clientinterface/notification/bulkcheckforupdates",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
	  ...
	  CURLOPT_POSTFIELDS => '{"clientstates":[{"accesstoken":"' . $accessToken . '","threadrevision":0,"activethreads":[]}]}',
	  CURLOPT_HTTPHEADER => array(
	    "cache-control: no-cache",
	    "content-type: application/json",
	  ),
...
function testLogout($chatURL, $accessToken, $deviceUUID) {
	$usePost = isset($_POST['usepost']) && $_POST['usepost'] == "true" ? true : false;
	
	$curl = curl_init();
	
	curl_setopt_array($curl, array(
	  CURLOPT_URL => $chatURL . "/index.php/wurrd/clientinterface/operator/logout",
	  CURLOPT_RETURNTRANSFER => true,
	  CURLOPT_ENCODING => "",
	  CURLOPT_MAXREDIRS => 10,
	  CURLOPT_TIMEOUT => 30,
	  ...
	  CURLOPT_HTTPHEADER => array(
	    "cache-control: no-cache",
	    "content-type: application/json",
	    "x-auth-token: " . $deviceUUID . ":" . $accessToken,
	  ),
	));
...
<head>
  ...
  <title>Wurrd Client Interface basic test</title>
  <meta name="description" content="Basic testing for the Wurrd client interface plugins">
...
<body>
<?php 
$chatURL = isset($_POST['chaturl']) ? $_POST['chaturl'] : null;
$username = isset($_POST['username']) ? $_POST['username'] : null;
$password = isset($_POST['password']) ? $_POST['password'] : null;
$deviceUUID = "aaa-uuu-isdi-php";
if (isset($chatURL) && count($chatURL) > 0 && 
...
<?php
	...
	$serverInfo = getServerInfo($chatURL);
	if ($serverInfo == null) {
		echo "Failed to get server info. Stopping tests here<br />";
		endTest(true);
	}
	
	echo "Chat server: " . $chatURL . "<br />";	
	echo "Chat platform: " . $serverInfo['chatplatform'] . "<br />";
	...
	$loginResults = testLogin($chatURL, $username, $password, $deviceUUID);
	...
	$checkForUpdatesResults = testCheckForUpdates($chatURL, $accessToken);	
	...
	$logoutResults = testLogout($chatURL, $accessToken, $deviceUUID);	
